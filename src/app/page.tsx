...
import ServerCan from "./ui/ServerCan";
import { PANELS, Role } from "@/lib/roles";
...
export default function Page({
  ...
  const role = (searchParams?.role ?? "patron") as Role;
        ...
        <div className="flex flex-col md:flex-row items-center gap-8">
          ...
          <div className="text-center md:text-left md:flex-1">
            ...
            <p className="text-base md:text-lg opacity-90 mt-3">
              Herkes için basit • Patron için net • orcyork@gmail.com
            </p>
            <p className="text-sm opacity-80 mt-1">
              Demo sürüm — geri bildirimleriniz değerli
            </p>
            <div className="mt-6 flex flex-wrap justify-center md:hidden gap-3">
              <a
                ...
                className="px-5 py-2 rounded-xl bg-black hover:bg-gray-900 text-white font-medium shadow-md transition"
              >
                GitHub Repo
              </a>
              <a
              ...
              <a
                href="/signup"
                className="px-5 py-2 rounded-xl bg-white/90 hover:bg-white text-emerald-700 font-semibold shadow-md transition ring-2 ring-white/40"
              >
                ÜYE OL
              </a>
            </div>
...
>>>>>>> feat/roles-kpis-tasks
      ...
      <RoleBar />
      <KpiStrip key={role} role={role} />
<ServerCan role={role} anyOf={PANELS.patronDashboard}>
      ...
      <div className="columns-1 md:columns-2 gap-4 [column-fill:_balance]">
        <ServerCan role={role} anyOf={PANELS.orders}>
          <section className="card p-4 mb-4 inline-block w-full break-inside-avoid">
            <Orders />
          </section>
        </ServerCan>

        <ServerCan role={role} anyOf={PANELS.shifts}>
          <section className="card p-4 mb-4 inline-block w-full break-inside-avoid">
            <Shifts />
          </section>
        </ServerCan>

        <ServerCan role={role} anyOf={PANELS.finance}>
          <section className="card p-4 mb-4 inline-block w-full break-inside-avoid">
            <Finance />
          </section>
        </ServerCan>

        <ServerCan role={role} anyOf={PANELS.inventory}>
          <section className="card p-4 mb-4 inline-block w-full break-inside-avoid">
            <Inventory />
          </section>
        </ServerCan>
        <ServerCan role={role} anyOf={PANELS.miniTasks}>
          <section className="card p-4 mb-4 inline-block w-full break-inside-avoid">
            <MiniTasks />
          </section>
          </ServerCan>
